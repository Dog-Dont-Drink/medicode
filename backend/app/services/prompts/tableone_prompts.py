"""Prompt builders for baseline Table 1 interpretation."""
...
def serialize_table(table: TableOneTablePayload) -> str:
...
def build_tableone_system_prompt(language: Literal["zh", "en"]) -> str:
    if language == "en":
        return (
...
def build_tableone_user_prompt(table: TableOneTablePayload, language: Literal["zh", "en"]) -> str:
    output_requirement = (
        "Write 1 to 2 compact, polished result paragraphs in English, suitable for an SCI manuscript Results section."
        if language == "en"
        else "请用中文写 1 段凝练、连贯的结果描述，语气符合 SCI 医学论文 Results 部分。"
    )
    additional_rules = (
