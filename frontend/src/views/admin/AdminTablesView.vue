            ...
            <div v-if="selectedRow" class="space-y-4">
              <div class="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p class="mt-2 text-sm font-medium text-slate-900">主键 {{ selectedRowId }}</p>
                </div>
                <button
                  type="button"
...
const currentTableLabel = computed(() => {
  ...
  return labelMap[tableName] || tableName.replaceAll('_', ' ') || '数据表'
})

const editableColumns = computed(() =>
  (currentTable.value?.columns || []).filter((column) => !column.primary_key)
)
