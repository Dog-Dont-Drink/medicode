    ...
    <nav class="scrollbar-hide flex-1 overflow-y-auto px-3 py-3">
      <div class="space-y-0.5">
        <router-link to="/dashboard" :class="linkClass('/dashboard')" @click="$emit('close')">
          <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/></svg>
          <span>工作台</span>
        </router-link>
        ...
        <div class="group space-y-0.5" @mouseenter="hoveredSection = 'data'" @mouseleave="hoveredSection = null">
          <router-link to="/data" :class="linkClass('/data')" @click="$emit('close')">
            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg>
            <span>数据管理</span>
          </router-link>
      ...
      <div class="mb-2 mt-5 px-2">
        <p class="text-[12px] font-medium tracking-wide text-gray-400">统计分析</p>
      </div>
      ...
      <div class="space-y-0.5">
        <div class="group space-y-0.5" @mouseenter="hoveredSection = 'basic'" @mouseleave="hoveredSection = null">
          <button class="nav-group-btn" type="button">
            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>
        ...
        <div class="group space-y-0.5" @mouseenter="hoveredSection = 'regression'" @mouseleave="hoveredSection = null">
          <button class="nav-group-btn" type="button">
            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v16a2 2 0 0 0 2 2h16"/><path d="M7 16c.5-2 1.5-7 4-7 2 0 2 3 4 3 2.5 0 4.5-5 5-7"/></svg>
        ...
        <div class="group space-y-0.5" @mouseenter="hoveredSection = 'advanced'" @mouseleave="hoveredSection = null">
          <button class="nav-group-btn" type="button">
            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M2 12c0-5.52 4.48-10 10-10"/></svg>
      ...
      <div class="mb-2 mt-5 px-2">
        <p class="text-[12px] font-medium tracking-wide text-gray-400">其他</p>
      </div>

      <div class="space-y-0.5">
        <div class="group space-y-0.5" @mouseenter="hoveredSection = 'polish'" @mouseleave="hoveredSection = null">
          <button class="nav-group-btn" type="button">
            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
              <path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z"/>
            </svg>
            <span class="flex-1 text-left">表达润色</span>
            <svg :class="['h-3.5 w-3.5 text-gray-300 transition-transform duration-200', isSectionExpanded('polish') && 'rotate-90']" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
          </button>
          <div v-if="isSectionExpanded('polish')" class="ml-[26px] space-y-0.5 border-l border-gray-100 pb-1 pl-3">
            <router-link
              v-for="item in polishItems"
              :key="item.path"
              :to="item.path"
              :class="subLinkClass(item.path, true)"
              @click="$emit('close')"
        ...
        <router-link to="/reports" :class="linkClass('/reports')" @click="$emit('close')">
          <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
          <span>报告中心</span>
        </router-link>

        <router-link to="/account/billing" :class="linkClass('/account/billing')" @click="$emit('close')">
          <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M13 2L4 14h6l-1 8 9-12h-6l1-8z"/></svg>
          <span>资源充值</span>
        </router-link>
      </div>
    </nav>
...
function subLinkClass(path: string, exact = false) {
  return [
    'block rounded-md px-2.5 py-1.5 text-[13px] transition-all duration-150 cursor-pointer',
...
const polishItems = [
  { path: '/analysis/polish/grammar', label: '语法校订' },
  { path: '/analysis/polish/manuscript', label: '结果成文' },
  { path: '/analysis/polish/background', label: '文献铺陈' },
  { path: '/analysis/polish/discussion', label: '结果析论' },
]
</script>
